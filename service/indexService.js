...
function getMenu(param,callback) {
    connect().query('select * from tz_uhome_back_abc where `type` != 2 order by order_num', function (error, result, fields) {
...
function getInfo(param,callback) {
    connect().query('SELECT * from tz_uhome_back_abd',function (error, results, fields) {
        if (error) throw error;
        callback(results);
    });
}
